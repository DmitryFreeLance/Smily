...
- `/burger` — alias для еда-режима
- `/dick` — попытка в писька-режиме (лимит 1 раз в день)
- `/menu` — открыть главное меню
- `/admin` — админ-панель (только для `ADMIN_IDS`)

В группах:
- бот удаляет сообщения со ссылками/доменами;
- игровые команды вызываются как `/burger@<bot_username>` и `/dick@<bot_username>`;
- на обычные сообщения бот не отвечает.
